...
tract = args[2]
scalar = args[3]
print(paste("Running NEST for", dataset, tract, scalar))
...
NEST_outputs_dir <- paste0(tract_prof_output_root, "/NEST/", scalar, "/")
...
NEST_wrapper <- function(tract, df, bin_size, nodes_to_clip) {
  ...
  df_wide <- df_tract %>% select(sub, tract_node, all_of(scalar)) %>% pivot_wider(names_from = "tract_node", values_from = scalar) 
...
df <- readRDS(sprintf("%1$s/tract_profiles/tract_profiles_for_NEST_noddi.RData", tract_prof_output_root))
