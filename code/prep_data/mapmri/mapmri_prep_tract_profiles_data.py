...
Output:
1) collated_tract_profiles_tmp.tsv for each dataset for subjects with ALL tracts (rows = 100 nodes * 28 tracts * X subjects; columns = 3 metrics + tractID + nodeID).
...
for dataset in ["HCPD", "HBN"]: 
    ...
    with open(config_file, "rb") as f:
        config = json.load(f)
    data_root = ospj(config['data_root'], "derivatives", "mapmri")
    output_root = f"{data_root}/all_subjects" 
    ...
    all_tract_profiles = pd.DataFrame(columns=['sub', 'tractID', 'nodeID', 'RTAP', 'RTOP', 'RTPP'])
    missing_tract_profiles = pd.DataFrame(columns=['sub', 'tractID', 'nodeID', 'RTAP', 'RTOP', 'RTPP'])
    ...
    for sub in os.listdir(data_root):
        sub_dir = os.path.join(data_root, sub, "TORTOISE_MAPMRI")
        if os.path.isdir(sub_dir) and sub != "all_subjects":
            for filename in os.listdir(sub_dir):
                if filename.endswith("desc-profiles_tractography.csv"):
    ...
    all_tract_profiles.to_csv(f'{output_root}/collated_tract_profiles_nocovbat_tmp.tsv', index=False)

    #missing_tract_profiles = missing_tract_profiles.iloc[:, :-1]
    missing_tract_profiles.to_csv(f'{output_root}/missing_tract_profiles_nocovbat.tsv', index=False)
    ...
    with open(os.path.join(sample_selection_dir, 'tortoise_subs_with_nans.txt'), 'w') as f:
