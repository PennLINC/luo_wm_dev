...
for dataset in ["HCPD", "HBN"]: 
    ...
    with open(config_file, "rb") as f:
        config = json.load(f)
    data_root = ospj(config['data_root'], "derivatives", "noddi")
    ...
    all_tract_profiles = pd.DataFrame(columns=['sub', 'tractID', 'nodeID', 'icvf', 'isovf', 'od'])
    missing_tract_profiles = pd.DataFrame(columns=['sub', 'tractID', 'nodeID', 'icvf', 'isovf', 'od'])
    ...
    for sub in os.listdir(data_root):
        sub_dir = os.path.join(data_root, sub, "wmNODDI")
    ...
    with open(os.path.join(sample_selection_dir, 'noddi_subs_with_nans.txt'), 'w') as f:
