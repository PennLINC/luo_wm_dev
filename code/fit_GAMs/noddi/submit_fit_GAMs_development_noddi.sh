...
for dataset in "${datasets[@]}"; do
  config_file="/cbica/projects/luo_wm_dev/two_axes/code/config/config_${dataset}.json"
    ...
    logs_dir="/cbica/projects/luo_wm_dev/two_axes/code/logs/fit_GAMs/${dataset}"
    if [ ! -d "${logs_dir}" ]; then
        mkdir -p ${logs_dir}
    fi
  ...
  for scalar in "${scalars[@]}"; do
      ...
      sbatch --job-name=${job_name} --nodes=1 --ntasks=1 --cpus-per-task=4 --mem=8G --time=6:00:00 --output=${logs_dir}/${job_name}_%j.out --error=${logs_dir}/${job_name}_%j.err --wrap="singularity run --cleanenv /cbica/projects/luo_wm_dev/two_axes/software/r_packages/r-packages-for-cubic_0.0.7.sif Rscript --save ${r_script} ${dataset} ${scalar}"
