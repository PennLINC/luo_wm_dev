...
"""
This script maps tortoise MAPMRI nifti files to tract profiles following this DIPY tutorial:
https://workshop.dipy.org/documentation/1.7.0/examples_built/17_streamline_analysis/afq_tract_profiles/

Note that I have to manually make sure the streamlines are oriented all in the same direction. It doesn't seem like pyAFQ
fully reoriented all streamlines. 
...
output:
- rtap, rtop, rtpp tract profiles for each tract for this subject  
"""
...
# config_file = "/cbica/projects/luo_wm_dev/two_axes/code/config/config_HBN.json"
# pyafq_dir = "/cbica/projects/luo_wm_dev/input/HBN/derivatives/babs_qsirecon_pyafq_allsubs_noACT/merge_ds"
...
out_dir = ospj(data_root, f"derivatives/mapmri", subject, "TORTOISE_MAPMRI") # can save tract profiles into same folder as volumetric data
...
for fpath in volumetric_files:
    ...
    if match:
        ...
        tortoise_dict[param] = {'data': data, 'affine': affine}
    else:
...
for param, tortoise_data in tortoise_dict.items():
    scalar_volume = tortoise_data['data']
    scalar_affine = tortoise_data['affine']
    profiles[param] = {}
...
df_profiles_wide.to_csv(ospj(out_dir, f"{subject}_tortoise_desc-profiles_tractography.csv"), index=False)
