...
for dataset in "${datasets[@]}"; do
    ...
    logs_dir="/cbica/projects/luo_wm_dev/two_axes/code/logs/scalars_to_tractprofiles/mapmri/${dataset}"
    if [ ! -d "${logs_dir}" ]; then
        mkdir -p ${logs_dir}
    fi
    ...
    sbatch --parsable \
        ...
        --job-name="mapmri_tractprofiles_${dataset}" \
        ./mapmri_tractprofiles.sh ${subjects_file} ${config_file} ${pyafq_dir} ${logs_dir} "mapmri_tractprofiles_${dataset}"
