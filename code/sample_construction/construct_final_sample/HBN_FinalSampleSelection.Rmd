...
```{r}
# this txt file was made with awk -F, '$8 == "True" { print $1 }' job_status.csv > ~/two_axes/input/HBN/sample_selection_files/subs_failed_babs_noACT.txt
failed_babs <- read.table("/cbica/projects/luo_wm_dev/two_axes/input/HBN/sample_selection_files/subs_failed_babs_noACT.txt", header=F) # these subjects are already excluded in collated_tract_profiles_nocovbat_tmp.tsv below. But just wanted to look at who failed babs
...
missing_tracts <- fread("/cbica/projects/luo_wm_dev/input/HBN/derivatives/tract_profiles_v2/all_subjects/missing_tract_profiles_nocovbat.tsv")
# load full tract profiles of everyone (except for the subjects in missing_tracts)
HBN_all <- fread("/cbica/projects/luo_wm_dev/input/HBN/derivatives/tract_profiles_v2/all_subjects/collated_tract_profiles_nocovbat_tmp.tsv") # takes a while to load
...
write.table(subjects_to_exclude, "/cbica/projects/luo_wm_dev/two_axes/input/HBN/sample_selection_files/subs_missing_tracts_noACT.txt", col.names=T, row.names=F, quote = TRUE)

sub_id <- sub_id[!sub_id$sub_id %in% subjects_to_exclude$sub,] %>% as.data.frame() %>% setNames("sub_id") # N = 1090
...
length(unique(HBN_all_with_allowed$sub)) # final N = 1051. Excluded 50 subjects for missing babs, freesurfer, and missing required tracts 

write.table(HBN_all_with_allowed, "/cbica/projects/luo_wm_dev/input/HBN/derivatives/tract_profiles_v2/all_subjects/collated_tract_profiles_nocovbat.tsv", row.names = FALSE)
...
write.table(sub_id, "/cbica/projects/luo_wm_dev/two_axes/input/HBN/subject_list/final_sample/HBN_WMDev_FinalSample.txt", col.names=T, row.names=F, quote = TRUE)
write.csv(merge(demographics.final, dwi.qc.final), "/cbica/projects/luo_wm_dev/two_axes/input/HBN/sample_selection_files/final_sample/HBN_WMDev_FinalSampleDemoQC.csv", quote = F, row.names = F)
```

## Demographics table
```{r}
final_sample <- read.csv("/cbica/projects/luo_wm_dev/two_axes/input/HBN/sample_selection_files/final_sample/HBN_WMDev_FinalSampleDemoQC.csv", header=T)
