...
write.table(sub_id, "/cbica/projects/luo_wm_dev/two_axes/input/HBN/subject_list/HBN_subject_list_babs_withACT.txt", col.names=T, row.names=F, quote = FALSE)
```

```{r}
# 57 subjects failed qsirecon
# this txt file was made with awk -F, '$8 == "True" { print $1 }' job_status.csv > ~/two_axes/input/HBN/sample_selection_files/subs_failed_babs_ACT.txt
failed_babs <- read.table("/cbica/projects/luo_wm_dev/two_axes/input/HBN/sample_selection_files/subs_failed_babs_ACT.txt", header=F)
sub_id <- sub_id[!sub_id$sub_id %in% failed_babs$V1,] %>% as.data.frame() %>% setNames("sub_id") # remove 57 participants (44 additional participants since some of them also failed freesurfer) from sample of 1088 due to failing qsiprep/qsirecon, new N = 1044
```
...
missing_tracts <- fread("/cbica/projects/luo_wm_dev/input/HBN/derivatives/tract_profiles_ACT_v2/all_subjects/missing_tract_profiles_nocovbat.tsv")
# load full tract profiles of everyone (except for the subjects in missing_tracts)
HBN_all <- fread("/cbica/projects/luo_wm_dev/input/HBN/derivatives/tract_profiles_ACT_v2/all_subjects/collated_tract_profiles_nocovbat.tsv")
...
write.table(subjects_to_exclude, "/cbica/projects/luo_wm_dev/two_axes/input/HBN/sample_selection_files/subs_missing_tracts_withACT.txt", col.names=T, row.names=F, quote = TRUE)

sub_id <- sub_id[!sub_id$sub_id %in% subjects_to_exclude$sub,] %>% as.data.frame() %>% setNames("sub_id") # remove 137 participants from sample of 1044 due to failing qsiprep/qsirecon, new N = 907
...
write.table(HBN_all_with_allowed, "/cbica/projects/luo_wm_dev/input/HBN/derivatives/tract_profiles_ACT_v2/all_subjects/collated_tract_profiles_nocovbat.tsv", row.names = FALSE)
```
...
write.table(sub_id, "/cbica/projects/luo_wm_dev/two_axes/input/HBN/subject_list/HBN_WMDev_FinalSample_withACT.txt", col.names=T, row.names=F, quote = TRUE)
write.csv(merge(demographics.final, dwi.qc.final), "/cbica/projects/luo_wm_dev/two_axes/input/HBN/sample_selection_files/HBN_WMDev_FinalSampleDemoQC_withACT.csv", quote = F, row.names = F)
```
