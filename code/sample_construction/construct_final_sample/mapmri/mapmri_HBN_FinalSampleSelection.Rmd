...
```{r}
missing_recon <- read.csv('/cbica/projects/luo_wm_dev/input/HBN/derivatives/mapmri/missing_files_HBN.csv')

missing_TORTOISE <- missing_recon %>% filter(str_detect(missing_file, "TORTOISE")) 

sub_id <- sub_id[!sub_id$sub_id %in% unique(missing_TORTOISE$subject),] %>% as.data.frame() %>% setNames("sub_id")  
...
sub_id <- sub_id[!sub_id$sub_id %in% unique(missing_freesurfer$sub_id),] %>% as.data.frame() %>% setNames("sub_id") # remove 13 subjects missing freesurfer, final N = 1088
...
missing_tracts <- fread("/cbica/projects/luo_wm_dev/input/HBN/derivatives/mapmri/all_subjects/missing_tract_profiles_nocovbat.tsv")
# load full tract profiles of everyone (except for the subjects in missing_tracts)
HBN_all <- fread("/cbica/projects/luo_wm_dev/input/HBN/derivatives/mapmri/all_subjects/collated_tract_profiles_nocovbat_tmp.tsv") # takes a while to load
...
write.table(subjects_to_exclude, "/cbica/projects/luo_wm_dev/two_axes/input/HBN/sample_selection_files/mapmri_subs_missing_tracts.txt", col.names=T, row.names=F, quote = TRUE)
...
write.table(HBN_all_with_allowed, "/cbica/projects/luo_wm_dev/input/HBN/derivatives/mapmri/all_subjects/collated_tract_profiles_nocovbat.tsv", row.names = FALSE) # N = 1051
```
...
write.table(sub_id, "/cbica/projects/luo_wm_dev/two_axes/input/HBN/subject_list/final_sample/HBN_WMDev_FinalSample_mapmri.txt", col.names=T, row.names=F, quote = TRUE)
write.csv(merge(demographics.final, dwi.qc.final), "/cbica/projects/luo_wm_dev/two_axes/input/HBN/sample_selection_files/final_sample/HBN_WMDev_FinalSampleDemoQC_mapmri.csv", quote = F, row.names = F)
```

## Demographics table
```{r}
final_sample <- read.csv("/cbica/projects/luo_wm_dev/two_axes/input/HBN/sample_selection_files/final_sample/HBN_WMDev_FinalSampleDemoQC_mapmri.csv", header=T)
...
identical(final_sample$sub, main_sample$sub)
```
