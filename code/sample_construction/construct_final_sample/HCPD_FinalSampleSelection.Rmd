...
# this txt file was made with awk -F, '$8 == "True" { print $1 }' job_status.csv > ~/two_axes/input/HCPD/sample_selection_files/subs_failed_babs.txt
fail_qsirecon <- read.table("/cbica/projects/luo_wm_dev/two_axes/input/HCPD/sample_selection_files/subs_failed_babs.txt", sep = ",")
...
missing_tracts <- fread("/cbica/projects/luo_wm_dev/input/HCPD/derivatives/tract_profiles_v2/all_subjects/missing_tract_profiles_nocovbat.tsv")
# load full tract profiles of everyone (except for the subjects in missing_tracts)
HCPD_all <- fread("/cbica/projects/luo_wm_dev/input/HCPD/derivatives/tract_profiles_v2/all_subjects/collated_tract_profiles_nocovbat_tmp.tsv") # takes a while to load
...
write.table(subjects_to_exclude, "/cbica/projects/luo_wm_dev/two_axes/input/HCPD/sample_selection_files/subs_missing_tracts.txt", col.names=T, row.names=F, quote = TRUE)

sub_id <- sub_id[!sub_id$sub_id %in% subjects_to_exclude$sub,] %>% as.data.frame() %>% setNames("sub_id") # remove 2 participant from sample of 569 due to failing qsiprep/qsirecon, new N = 567
```
...
write.table(HCPD_all_with_allowed, "/cbica/projects/luo_wm_dev/input/HCPD/derivatives/tract_profiles_v2/all_subjects/collated_tract_profiles_nocovbat.tsv", row.names = FALSE)
```
...
write.table(sub_id, "/cbica/projects/luo_wm_dev/two_axes/input/HCPD/subject_list/final_sample/HCPD_WMDev_FinalSample.txt", col.names=T, row.names=F, quote = TRUE)
write.csv(merge(demographics.final, dwi.qc.final), "/cbica/projects/luo_wm_dev/two_axes/input/HCPD/sample_selection_files/final_sample/HCPD_WMDev_FinalSampleDemoQC.csv", quote = F, row.names = F)
```

## Demographics table
```{r}
final_sample <- read.csv("/cbica/projects/luo_wm_dev/two_axes/input/HCPD/sample_selection_files/final_sample/HCPD_WMDev_FinalSampleDemoQC.csv", header=T)
