...
```{r}
sub_id <- read.table("/cbica/projects/luo_wm_dev/two_axes/input/PNC/subject_list/PNC_subject_list_babs.txt", header = T)
demographics.temp <- read.csv("/cbica/projects/luo_wm_dev/two_axes/input/PNC/sample_selection_files/PNC_WMDev_TempSampleDemographics_N1145_age8to23.csv")
dwi.qc.temp <- read.csv("/cbica/projects/luo_wm_dev/two_axes/input/PNC/sample_selection_files/PNC_WMDev_TempSampleQCMetrics_N1145_age8to23.csv")
```

## Incomplete Data Processing Exclusion
total excluded for incomplete data processing = 43
```{r}
# 43 subjects failed qsirecon
# this txt file was made with awk -F, '$8 == "True" { print $1 }' job_status.csv > ~/two_axes/input/PNC/sample_selection_files/subs_failed_babs.txt
failed_babs <- read.table("/cbica/projects/luo_wm_dev/two_axes/input/PNC/sample_selection_files/subs_failed_babs.txt", header=F)
...
# PNC isn't missing any freesurfer data
```
...
missing_tracts <- fread("/cbica/projects/luo_wm_dev/input/PNC/derivatives/tract_profiles_v2/all_subjects/missing_tract_profiles_nocovbat.tsv")
# load full tract profiles of everyone (except for the subjects in missing_tracts)
PNC_all <- fread("/cbica/projects/luo_wm_dev/input/PNC/derivatives/tract_profiles_v2/all_subjects/collated_tract_profiles_nocovbat_tmp.tsv") # takes a while to load
...
write.table(subjects_to_exclude, "/cbica/projects/luo_wm_dev/two_axes/input/PNC/sample_selection_files/subs_missing_tracts.txt", col.names=T, row.names=F, quote = TRUE)

sub_id <- sub_id[!sub_id$sub_id %in% subjects_to_exclude$sub,] %>% as.data.frame() %>% setNames("sub_id") # remove 4 participants from sample of 1102 due to failing qsiprep/qsirecon, new N = 1098
...
subjects_with_allowed_missing_tracts <- missing_tracts_long %>%
  filter(!sub %in% subjects_to_exclude$sub) %>%
  filter(str_detect(missing_tracts, paste(allowed_missing_tracts, collapse = "|")))
...
length(unique(PNC_all_with_allowed$sub))
write.table(PNC_all_with_allowed, "/cbica/projects/luo_wm_dev/input/PNC/derivatives/tract_profiles_v2/all_subjects/collated_tract_profiles_nocovbat.tsv", row.names = FALSE)
...
write.table(sub_id, "/cbica/projects/luo_wm_dev/two_axes/input/PNC/subject_list/final_sample/PNC_WMDev_FinalSample.txt", col.names=T, row.names=F, quote = TRUE)
write.csv(merge(demographics.final, dwi.qc.final), "/cbica/projects/luo_wm_dev/two_axes/input/PNC/sample_selection_files/final_sample/PNC_WMDev_FinalSampleDemoQC.csv", quote = F, row.names = F)
```

## Demographics table
```{r}
final_sample <- read.csv("/cbica/projects/luo_wm_dev/two_axes/input/PNC/sample_selection_files/final_sample/PNC_WMDev_FinalSampleDemoQC.csv", header=T)
